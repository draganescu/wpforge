...
### Added
- **`wpforge fix <dir>` — a vision-driven visual repair pass.** The coder model writes CSS blind; this hands the whole repair to a Gemini vision model that both sees the defects and writes the fix. It boots the generated site in WordPress Playground (PHP-WASM + SQLite, no local WP), screenshots one page per template surface with headless Chromium (desktop + mobile, incl. every CPT's archive/single; reveal-motion forced to rest, admin bar hidden), diagnoses GLARING CSS-fixable defects (contrast, overflow, overlap, unstyled blocks — taste and "missing content" excluded), then authors a CSS override patch **multimodally** — Gemini is shown the broken screenshots together with the current `style.css`, so it targets the real offending selectors — appends it to `style.css`, and re-screenshots on the same seeded server to verify (up to two passes). CSS-only by design; screenshots saved to `<output>/.wpforge-preview/`. Every generated output now ships a deterministic `wpforge-preview-blueprint.json` so the site can be booted in Playground anytime. New deps: `puppeteer`, `@wp-playground/cli`. New flags: `--vision-model`, `--port`, `--wp`; new env `WPFORGE_VISION_MODEL`. Needs only `GEMINI_API_KEY` (no Cerebras).
